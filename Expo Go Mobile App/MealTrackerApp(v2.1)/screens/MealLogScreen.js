...
import { sendLocalSummaryNotification } from '../lib/notifications';
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'meals',
          // NOTE: No server-side filter here — that requires Supabase row-level
          // filtering to be explicitly enabled in the dashboard (Replication settings).
          // Instead we filter client-side in the callback below.
        },
        (payload) => {
          // Client-side guard: only process events for this user
          const rowUserId = String(
            payload.new?.user_id ?? payload.old?.user_id ?? ''
          );
          if (rowUserId !== String(TELEGRAM_USER_ID)) return;
    ...
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          ...
          {/* Summary button — fires an instant local notification with today's tally */}
          <TouchableOpacity
            ...
            onPress={async () => {
              const today = new Date().toISOString().split('T')[0];
              const todayMeals = meals.filter((m) => m.date === today);
              const types = [...new Set(todayMeals.map((m) => m.meal_type?.toLowerCase()).filter(Boolean))];
              await sendLocalSummaryNotification(todayMeals.length, types);
              Alert.alert('📊 Summary Sent', 'Check your notification tray!');
            }}
          >
